...
def getPictures(groupName,showName):
    ...
    try:
        ...
        # groupName = '博士论坛'
        tblName = 't_'+ groupName +'\n'

        sql = 'select 表情数 from '+ tblName + 'order by 表情数 desc';
        cur.execute(sql)
        result = cur.fetchall()
        # 异常处理
...
#'''
    # 2、将表内数据结果赋值给变量
    PLNum = np.arange(1,len(result)+1,1)  # 序号
    Num = [n[0] for n in result]  # 表情数
    ...
    # 7.3 绘图、保存并显示
    plt.loglog(PLNum,Num,'ks',label='表情数')
    plt.plot(x,y,'k',label='拟合值')
    plt.legend()
    plt.savefig('image/'+showName +'.png')
    plt.show()
#'''
